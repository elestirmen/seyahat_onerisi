        ...
        async function getRecommendations() {
            ...
            try {
                ...
                if (window.loadingManager && window.loadingManager.performanceMonitor && typeof window.loadingManager.performanceMonitor.measureRender === 'function') {
                    try {
                        recommendations = window.loadingManager.performanceMonitor.measureRender(
                            'calculateRecommendations',
                            () => calculateRecommendations(poisData, preferences)
                        );
                    } catch (error) {
                        console.warn('Performance monitoring error:', error);
                        recommendations = calculateRecommendations(poisData, preferences);
                    }
                } else {
                ...
                // Initialize map with recommendations
                if (recommendations.length > 0) {
                    console.log('Initializing map with recommendations...');
                    await initializeMap(recommendations);
                } else {
