...
def main(category):
    ...
    for cat, items in pois.items():
        fg = folium.FeatureGroup(name=cat.capitalize())
    ...
    G = load_graph()
    coords = [coord for _, coord in pois[category].items()]
    route, length = compute_category_route(G, coords)
    route_name = f"{category.capitalize()} Rota ({length:.2f} km)"
    folium.PolyLine(route, color="purple", weight=5, opacity=0.7, tooltip=route_name).add_to(m)
    plugins.Draw(export=False).add_to(m)
    folium.LayerControl(collapsed=False).add_to(m)
    output = f"{category}_route.html"
    m.save(output)
    print(f"Harita '{output}' olarak kaydedildi.")
    print("POI noktaları:")
    for name in pois[category].keys():
        print(f"- {name}")
    print(f"Toplam uzunluk: {length:.2f} km")
